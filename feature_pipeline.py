...
def fetch_openweather_pollution() -> dict:
    ...
    Used as a fallback when AQICN data looks suspicious or is missing.
    """
...
def safe_positive(value) -> float:
    """Convert a value to float only if it is positive, otherwise return 0."""
    try:
        value = float(value)
    except (TypeError, ValueError):
        return 0.0
...
def fetch_recent_history(fg, city: str, hours: int = 72) -> pd.DataFrame:
    """
    Fetch recent rows from Hopsworks to compute live rolling and lag features.
    hours=72 so we always have enough history for aqi_lag_72h.
    """
    try:
        df = fg.read()
...
def compute_rolling_features(history_df: pd.DataFrame, current_aqi: float) -> dict:
    """
    Compute rolling averages, change rate, and lag features.
 
    Lag features (aqi_lag_24h, aqi_lag_48h, aqi_lag_72h) look back into
    Hopsworks history. They are the strongest predictors for 72h forecasting
    because AQI has strong persistence — knowing what AQI was 24/48/72h ago
    tells the model how a pollution episode is evolving.
    """
    now_ts = pd.Timestamp.utcnow()
 
    if history_df.empty:
        cached_aqi = get_cached_or_default("aqi")
        print("[!] No recent history; using cache/default fallback for rolling + lags.")
        return {
            "aqi_change_rate": round(current_aqi - cached_aqi, 4),
            "aqi_rolling_6h":  round(current_aqi, 4),
            "aqi_rolling_24h": round(current_aqi, 4),
            # FIX: lag features — fall back to current AQI if history unavailable
            "aqi_lag_24h": round(cached_aqi, 4),
            "aqi_lag_48h": round(cached_aqi, 4),
            "aqi_lag_72h": round(cached_aqi, 4),
        }
    ...
    aqi_series = combined.set_index("timestamp")["aqi"].astype(float)
 
    # Change rate: difference from previous row
    change_rate = float(aqi_series.iloc[-1] - aqi_series.iloc[-2]) if len(aqi_series) >= 2 else 0.0
 
    rolling_6h  = float(aqi_series.tail(6).mean())
    rolling_24h = float(aqi_series.tail(24).mean())
 
    def lag_value(hours: int) -> float:
        """Find the AQI value closest to `hours` ago in history."""
        target_ts = now_ts - pd.Timedelta(hours=hours)
        if aqi_series.empty:
            return get_cached_or_default("aqi")
        # Pick the row nearest to target_ts
        idx = (aqi_series.index - target_ts).to_series().abs().idxmin()
        return float(aqi_series[idx])
 
    lag_24h = lag_value(24)
    lag_48h = lag_value(48)
    lag_72h = lag_value(72)
 
    print(
        f"[✓] Rolling: change={change_rate:.2f}, "
        f"6h={rolling_6h:.2f}, 24h={rolling_24h:.2f} | "
        f"Lags: 24h={lag_24h:.2f}, 48h={lag_48h:.2f}, 72h={lag_72h:.2f}"
    )
    ...
    return {
        ...
        "aqi_lag_24h":     round(lag_24h, 4),
        "aqi_lag_48h":     round(lag_48h, 4),
        "aqi_lag_72h":     round(lag_72h, 4),
    }
...
def engineer_features(
    ...
    return {
        ...
        "wind_deg":    wind_deg,
        "wind_sin":    float(np.sin(np.radians(wind_deg))),
        "wind_cos":    float(np.cos(np.radians(wind_deg))),
        "hour":        int(hour),
        "day_of_week": int(day),
        "month":       int(month),
        ...
        "aqi_lag_24h": float(rolling["aqi_lag_24h"]),
        "aqi_lag_48h": float(rolling["aqi_lag_48h"]),
        "aqi_lag_72h": float(rolling["aqi_lag_72h"]),
        # Live future targets are unknown — kept None to avoid leakage
        "target_aqi_3h":  None,
        "target_aqi_24h": None,
        "target_aqi_72h": None,
    }
...
def connect_hopsworks():
    ...
    fg = fs.get_or_create_feature_group(
        ...
        description="Hourly AQI + weather features with forecasting targets (v2: wind encoding + lag features)",
...
def run() -> None:
    print("=" * 60)
    print(" Feature Pipeline — v2 (wind encoding + lag features)")
    print("=" * 60)
    ...
    print("[✓] Hopsworks connected (feature group v2)")
    print()
 
    print("▶ Step 2: Fetching recent Hopsworks history (72h)...")
    history_df = fetch_recent_history(fg, CITY, hours=72)
    print()
    ...
    print("▶ Step 7: Computing rolling + lag features...")
    ...
    print(
        ...
        f"wind_sin={features['wind_sin']:.3f}, wind_cos={features['wind_cos']:.3f}, "
        f"lag_24h={features['aqi_lag_24h']}, lag_48h={features['aqi_lag_48h']}, "
        f"lag_72h={features['aqi_lag_72h']}"
    )
    print()
